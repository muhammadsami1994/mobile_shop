...
mobileShop.factory('AutoCompleteService', function($q, $timeout) {

    var search = function(searchFilter,filterData,searchType) {

        console.log('Searching airlines for ' + searchFilter);

        var deferred = $q.defer();

        var matches = filterData.filter(function(data) {
            if(searchType=="categorySearch")
                if (data.toLowerCase().indexOf(searchFilter.toLowerCase()) !== -1) return true;
            if(searchType=="storeSearch")
                if (data.storeName.toLowerCase().indexOf(searchFilter.toLowerCase()) !== -1) return  true;
        });
